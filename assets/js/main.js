...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            // Si el formulario no tiene action de FormSubmit, prevenir el envío
            const action = this.getAttribute('action');

            if (!action || !action.includes('formsubmit.co')) {
                e.preventDefault();
                showFormMessage('Por favor, configura el correo electrónico de destino.', 'error');
                return;
            }
            ...
            // El formulario se enviará naturalmente con FormSubmit
            // FormSubmit redirigirá automáticamente a la página de agradecimiento
        });
    }
